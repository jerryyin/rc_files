...
3. **Answer "why" before "what".** The user's first instinct is "why is this
   needed?" or "why can't we just reuse X?" Address the root cause before
   proposing or explaining an implementation.

4. **Cross-vendor comparison is mandatory.** For any AMD-specific lowering,
   proactively reference how NVIDIA handles the same operation. Cite the
   specific file/function. Explain what's shared, what diverges, and why.

5. **Minimal, not comprehensive.** Tests, examples, and IR snippets should
   contain only the ops under discussion. Strip boilerplate unless relevant.

6. **Hardware semantics matter.** When discussing an IR-level concept (pred,
   barrier, encoding), always clarify:
   - What level it operates at (workgroup / warp / lane / register)
   - How the LLVM lowering refines or transforms it
...
7. For `SliceEncodingAttr`: project out the sliced dim — all threads/warps
   along that dim become broadcast
